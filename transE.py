...
workbook = xlsxwriter.Workbook('C:/Users/1/Desktop/loss_1000.xls')
...
class TransE:
    ...
    def train(self, epochs):
        global num_cols
        batch_size = 400
        nbatches = len(self.triple_list) // batch_size
        ...
        for epoch in range(epochs):
            ...
            logging.info("cost time:={}".format(time_cost))
            logging.info("loss:={}".format(self.loss))
            cols_loss.append(self.loss)
            cols_loss.append(time_cost)
            worksheet.write(num_cols, 0, num_cols)
            worksheet.write(num_cols, 1, self.loss)
            worksheet.write(num_cols, 2, time_cost)
...
if __name__ == '__main__':
    ...
    logging.info("Complete load. entity : %d , relation : %d , triple : %d" % (
